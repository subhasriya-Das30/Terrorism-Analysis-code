# -*- coding: utf-8 -*-
"""
Created on Sun Sep  6 10:20:44 2020
...
# A function to load data into df,read csv data and to get some insigts of values
def load_data():
  dataset_name = "GLOBAL_TERRORISM_01.csv "
  ...
  global country_list               # Total 205 Countries
...
# A function for Layout of the page
def create_app8_ui():
  # UI of the Page
  main_layout = html.Div(id='backgroundimage',className='backgroundimage',children=
    [html.Br(),
    html.H1('Terrorism Analysis with Insights',id='main_header',
           className='header'),
    
    
  # MAP TAB UI (WORLD MAP TAB AND INDIA MAP TAB)
  dcc.Tabs(id="Tabs", value="Map",className='tab',children=[
      
      dcc.Tab(label="Map tool" ,id="Map tool",value="Map",className='tabmap', children=[
          
          dcc.Tabs(id = "subtabs", value = "WorldMap",className='map',children = [
              
              dcc.Tab(label="World Map tool", id="World", value="WorldMap"),#className='maptools'),
              
              dcc.Tab(label="India Map tool", id="India", value="IndiaMap",className='maptools')
              ]),
      html.Br(),
          
          # WORLD MAP DROPDOWN UI WITH INLINE STLING USING CSS
          dcc.Dropdown(
              id='month', 
                options=month_list,
          ...
          dcc.Dropdown(
                ...
                style={'display': 'inline-block',
                       ...
                       'padding': '20 px',
                       'font-size': '16px',
                       'font-color':'white','cursor': 'pointer'
}
                  ),
          dcc.Dropdown(
      ...
      # CHART TAB UI (WORLD MAP CHART AND INDIA MAP CHART )
      dcc.Tab(label = "Chart Tool", id="chart tool", value="Chart",className='tabmap', children=[
          
          dcc.Tabs(id = "subtabs2", value = "WorldChart",className="map",children = [
              
            dcc.Tab(label="World Chart tool", id="WorldC", value="WorldChart",className='charttools'),
              
            dcc.Tab(label="India Chart tool", id="IndiaC", value="IndiaChart",className='charttools')]),
                                             html.Br(),
           ...
           dcc.Dropdown(id="Chart_Dropdown", options = chart_dropdown_values,
                         ...
                         #INLINE STYLING OF CHART DROPDOWN
                        style=
                         {'display':'inline-block'
                         ,'height': '30px','width': '100%',
                         'padding': '10 px','borderRadius':'30px','cursor': 'pointer'}),
            html.Hr(),
            html.Br(),
            ...
            html.Br(),
            # CHART TOOL RANGE SLIDER UI
            dcc.RangeSlider(                       
                    id='cyear_slider',
                    min=min(year_list),
                    max=max(year_list),
                    value=[min(year_list),max(year_list)],
                    marks=year_dict,#style={'color':'black'},
                    step=None
                      ),
                  html.Br()
              ]),
...
# CALLING OF THE CALLBACKS VALUES INTO THIS FUNCTION
def update_app9_ui(Tabs, month_value, date_value,region_value,country_value,state_value,city_value,attack_value,year_value,chart_year_selector, chart_dp_value, search,
    ...
    if Tabs == "Map":
        ...
        if attack_value == [] or attack_value is None:
            ...
            #all the operations are done on new_df and stored in new_df 
            
         #setting the figure for blank, since the callbacks called once when it runs for first time        
        mapFigure = go.Figure()
        if new_df.shape[0]:
            pass
        else: 
        ...
        # UPDATING THE EMPTY GRAPH WITH DATAS
...
#UPDATING CHART DATAS 
        ...
        fig = chartFigure
    return dcc.Graph(figure = fig,style={'height':'700','color':'black'})


#CALLBACK FOR INTERRELATING MONTH WITH DATES
@app.callback(
  Output("date", "options"),
  [Input("month", "value")])
def update_date(month):
    ...
    return option
#CALLBACK  FOR INTERRELATING REGION AND COUNTRY IN MAPTAB
...
#CALLBACK FOR INTERRELATING REGION AND COUNTRY DROPDOWN
@app.callback(
...
#CALLBACK  FOR INTERRELATING STATE AND COUNTRY DROPDOWN
@app.callback(
...
def set_state_options(country_value):
  # Making the state Dropdown data
    ...
    return [{'label':m , 'value':m} for m in option]
#CALLBACK  FOR INTERRELATING STATE AND CITY DROPDOWN
@app.callback(
...
# FLOW OF PROJECT OR THE EXECUTION HAPPENS THIS WAY
def main8():
  load_data()
  ...
  app.title = "Terrorism Analysis with Insights" #TITLE OF THE PROJECT
